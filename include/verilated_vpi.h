...
public:
	...
	*((void**)oldp) = s_freeHead;
    ...
    virtual const VerilatedRange* range() { return 0; }
...
public:
    ...
    virtual const VerilatedRange* range() const { return m_range; }
...
protected:
    ...
    vlsint32_t			m_index;
    const VerilatedRange&       get_range() { return (m_varp->dims()>1)?m_varp->array():m_varp->range(); }
public:
...
vpiHandle vpi_handle(PLI_INT32 type, vpiHandle object) {
	...
	if (VL_UNLIKELY(!vop->range())) return 0;
        return (new VerilatedVpioConst(vop->range()->left()))->castVpiHandle();
    }
    case vpiRightRange: {
	VerilatedVpioVar* vop = VerilatedVpioVar::castp(object);
	if (VL_UNLIKELY(!vop)) return 0;
	if (VL_UNLIKELY(!vop->range())) return 0;
        return (new VerilatedVpioConst(vop->range()->right()))->castVpiHandle();
    }
    case vpiIndex: {
	VerilatedVpioVar* vop = VerilatedVpioVar::castp(object);
	if (VL_UNLIKELY(!vop)) return 0;
...
vpiHandle vpi_iterate(PLI_INT32 type, vpiHandle object) {
	...
	return ((new VerilatedVpioRange(vop->range()))->castVpiHandle());
    }
    case vpiReg: {
